...
class Valas extends MY_Controller {
	...
	public function create_new() {
		...
		$save_result =  $this->valas_model->create_new($params);
	...
	public function get_detail()
	{
		...
		$attachments = $this->valas_model->get_attachments($header_id);
	...
	public function print_doc($id) {
		...
		$this->load->helper('my_helper');
		$data = array();
		$header_id = $this->my_decrypt($id);
		$data =  $this->get_data($header_id);
		$this->valas_model->close_doc($data['personal']->id);
		
		$this->load->view('valas/print_page', $data);
	}

	public function upload_file() {
		$config['upload_path']          = './assets/custom/valas/';
        $config['allowed_types']        = 'jpg|png|jpeg';
        $config['max_size']             = 2000;
        $config['file_name']            = 'VALAS_' . time() . '_' . rand(1, 1000) . '.jpg';
		...
		if ($this->upload->do_upload('file')) {
			...
			$this->valas_model->save_valas_attachments($config['file_name'], $header_id, $remark);
        } else {
	...
	{
		$data =  $this->valas_model->get_detail($header_id);
