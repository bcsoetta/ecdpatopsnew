...
class Departure_model extends MY_Model {
    ...
    public function get_departure($header_id) {
        ...
        $this->db->select('
			A.office, 
			A.doc_number, 
			A.created_date AS date_registered, 
			A.flight_number, 
			A.arrival_date, 
			A.last_port,
            A.name, 
		...
		$this->db->join('countries C', 'A.id_country = C.id');
    ...
    public function get_detail($header_id) {
        ...
        if ($reason == '2') {
			$this->db->select('
				...
				A.postal_code
			');
            $this->db->from('dep_valas_others A');
			$this->db->join('countries B', 'A.nationality = B.id', 'left');
	...
	public function save_departure_attachments($fileName, $header_id, $remark)
	{
		$this->db->set('name', $fileName);
		$this->db->set('header_id', $header_id);
		$this->db->set('jenis', $remark);
		$this->db->insert('dep_valas_attachment');
	}
	...
	public function get_attachments($header_id)
	{
		...
		$this->db->where('A.header_id', $header_id);
		$this->db->from('dep_valas_attachment A');
