(function($) {
    var Departure = {
		...
		getDetail: function() {
			...
			var msg = {
				header_id: Departure.myEncrypt(data)
			}
	
			$.ajax({
				url: '/departure/get_detail',
				type: 'post',
				dataType: 'json',
				...
				if (result) {
					...
					var permit_date = new Date(dataPersonal.permit_date);
					permit_date = permit_date.getDate() + '-' +
						(permit_date.getMonth()+1) + '-' +
						permit_date.getFullYear();
					var is_permitted = (dataPersonal.is_permit_bank === '1') ? `Ada (${dataPersonal.permit_number} tanggal ${permit_date})` :'Tidak ada';
					...
					dataAttachment.forEach(data => {
						...
						row = `
							<tr>
								...
								<td>${data['jenis']}</td>
								<td><a href="assets/custom/departure/${data['name']}" target="blank">${data['name']}</a></td>
							</tr>
						`
        ...
        renderData: function(data) {
            ...
            $.each(data.rows, function(index, value) {
				...
				row.find('[view="actionReview"]').on('click', Departure.getDetail);
        ...
        clearContent: function() {
			...
			// Reset params
			Departure.params.otherAttachmentNumber = 0;
			Departure.params.files = [];
        },
        createNew: function() {
		...
		verifyFiles: function() {
			...
			if (filePassport) {
				let fileValidity = Departure.verifyFile(filePassport, 'PASSPORT');
			...
			if (fileKtp) {
				let fileValidity = Departure.verifyFile(fileKtp, 'KTP');
				if (fileValidity == false) { return false };
			}
			...
			if (filePermit) {
				let fileValidity = Departure.verifyFile(filePermit, 'IZIN BI');
				if (fileValidity == false) { return false };
			}
			...
			$('div#otherAttachments > div.anotherAttachment').each(function () {
				...
				if (fileAttachment) {
					let fileValidity = Departure.verifyFile(fileAttachment, fileRemark);
		...
		verifyFile: function(fileData, remark) {
			if (fileData.size > 0) {
				if (fileData.size > 2000000) {
					...
					if (Departure.params.confirmSave == 1) {
						Departure.params.files.push({remark: remark, file: fileData});	
					}
				}
			}
		},
		uploadFiles: function (header_id) {
			Departure.params.files.forEach(upload => {
				Departure.uploadFile(upload, header_id);
			});
		},
        ...
        init: function() {
			...
			$('[name="cashForm"]').on('submit', function() {
				...
				var row = template.clone().removeClass('d-none').removeAttr('template');
				row.find('[view="cashNumber"]').html(Departure.params.cashNumber);
				...
				row.find('[view="cashAction"]').on('click', Departure.removeItems);
	
				row.appendTo(rows);
				Departure.params.itemDetailNumber = Departure.params.cashNumber + 1;
			...
			$(document).on("change", "input[name='is_bank_permit']", function () {
				if ($(this).val() == '0') {
					// Detail
					$('input#permitNumber').val('');
					$('input#permitDate').val('');
					$('input#permitNumber').prop('required', false);
					$('input#permitDate').prop('required', false);
					$('div#permitDetail').hide();

					// Lampiran
					$('input#attachPermit').prop('required', false);
					$('div#permitAttachment').hide();
				} else {
					// Detail
					$('input#permitNumber').prop('required', true);
					$('input#permitDate').prop('required', true);
					$('div#permitDetail').show();
					
					// Lampiran
					$('input#attachPermit').prop('required', true);
					$('div#permitAttachment').show();
				}
			});

			// Add new attachment
			$('#btnAddAttachment').on('click', function () {
				Departure.params.otherAttachmentNumber++
				let attachmentNumber = Departure.params.otherAttachmentNumber
            ...
            $('#dateFrom, #dateUntil, #birth, #other_birth, #arrival_date, #instrumentDate, #permitDate').datepicker({
                todayHighlight: true,
            ...
            $('form[name="newValasFormStep2"]').on('submit', function() {
				...
				let fileValidity = Departure.verifyFiles()
				if (!fileValidity) {
					return false
				}
